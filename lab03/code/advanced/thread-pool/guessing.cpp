...
void ThreadPool::run()
{
    while (true) {
        TaskBase* task = nullptr;
        {
            ...
            condition.wait(lock, [&] { return stop_flag || !tasks.empty(); });
...
void PriorityQueue::submit_task(segment* a,const string& pre_terminal,int start_index,int end_index)
{
    remaining_tasks++;
    pool.enqueue(new Fill_Task(this,a,pre_terminal,start_index,end_index));
}
        ...
        // 这个过程是可以高度并行化的
            ...
            if(all >= Threadshold)
            {
                ...
                for(int i = 0;i < THREAD_NUM;i++)
                {
                    ...
                    int end_index = (i == THREAD_NUM - 1) ? all : (i + 1) * chunk;
                    submit_task(a,"",start_index, end_index);
                }
        ...
        // 这个过程是可以高度并行化的
            ...
            if(all >= Threadshold)
            {
                ...
                for(int i = 0;i < THREAD_NUM;i++)
                {
                    int start_index = i * chunk;
                    int end_index = (i == THREAD_NUM - 1) ? all : (i + 1) * chunk;
                    submit_task(a,guess,start_index, end_index);
                }
                std::unique_lock<std::mutex> lock(done_mutex);
                done_condition.wait(lock, [&] 
                {
