...
int main()
{
    ...
    ofstream chain_add_2_algo("sum_chain_2_algo.txt");
    ofstream chain_add_4_algo("sum_chain_4_algo.txt");
    ofstream chain_add_8_algo("sum_chain_8_algo.txt");
    ofstream recursion_algo("sum_recursion_algo.txt");
    ...
    for(int n = 8;n < N;n = n * 2)
    {
        ...
        avg_time = elapsed * 0.01;
        chain_add_2_algo<<n<<" "<<avg_time<<endl;
        ...
        elapsed = seconds * 1e6 + micro_seconds;
        avg_time = elapsed * 0.01;
        chain_add_4_algo<<n<<" "<<avg_time<<endl;
        ...
        elapsed = seconds * 1e6 + micro_seconds;
        avg_time = elapsed * 0.01;
        chain_add_8_algo<<n<<" "<<avg_time<<endl;
        for(int cnt = 0;cnt < 100;cnt++)
        {
            for(int i = 0;i < n;i++)a[i] = 2 * i + 1;
            gettimeofday(&start,nullptr);
        ...
        avg_time = elapsed*0.01;
        recursion_algo<<n<<" "<<avg_time<<endl;
    }
    return 0;
}